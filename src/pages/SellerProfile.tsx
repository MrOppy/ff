...
export default function SellerProfile() {
    ...
    useEffect(() => {
        async function fetchSellerListings() {
            ...
            try {
                const data = await listingService.getListingsBySeller(sellerName);
                if (data) {
                    setListings(data);
                }
            } catch (err) {
        ...
        async function fetchSellerProfile() {
            ...
            try {
                ...
                if (!snap.empty) {
                    ...
                    // Fallback to basic seller if not found in users collection
        ...
        Promise.all([fetchSellerListings(), fetchSellerProfile()]).finally(() => setLoading(false));
    }, [sellerName]);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
                        ...
                        <div className="flex-1 text-center md:text-left">
                            <h1 className="text-4xl font-heading font-extrabold text-white mb-2 flex items-center justify-center md:justify-start gap-3">
                                {sellerName} <ShieldCheck className="w-8 h-8 text-emerald-500" />
                            </h1>
                            {/* Role Badge & Reviews */}
                            ...
                            <div className="flex flex-wrap justify-center md:justify-start gap-3 mt-4">
                                <div className="bg-gaming-900 border border-gaming-700 px-4 py-2 rounded-lg flex items-center gap-2">
                                    ...
                                    <span className="text-sm font-bold text-white">{displayListings.length} All Listings</span>
                                </div>
                            </div>
                ...
                <div className="mb-8 flex items-center justify-between">
                    <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                        <Package className="w-6 h-6 text-gaming-accent" /> All Listings ({displayListings.length})
                    </h2>
                </div>
